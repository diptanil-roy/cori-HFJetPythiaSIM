...
int    _runId   = 0;
int    _eventId = 0;
void trig( int runId, int eventId, double vx, double vy, double vz )
{
  ...
  if ( config=="pp:heavyflavor:D0jets" ) 
    {
      ...
      // pythia8->Set("421:onMode = 0");
      // pythia8->Set("421:onIfMatch = -321 211");
      // While the next two lines might work, I am not sure this is the right thing to do. However, for the sake of posterity, we need to check all possibilities.
      // pythia8->Set("321:mayDecay = 0"); //Let's see if this works.
      // pythia8->Set("211:mayDecay = 0"); //Let's see if this works.
...
void getNextRun( int& event, int& run, double& x, double& y, double& z ) {
...
void starsim( Int_t nevents=1, Int_t runnumber=15117062, TString runfile_="15117062_0000", int sequence=0, TString dummy = "0000", double ckin3 = 3.0, double ckin4 = 0.0 )
{ 
  ...
  const int stride = 1;
  const int offset = dummy.Atoi() + 20000000;

  int     rngSeed = runnumber * stride + sequence + offset;
  ...
  cout << "This is the file = " << runfile.Data() << endl;
  cout << "Seed is = " << rngSeed << endl;
...
//   }
  ...
  //
  // double ckin3=0.0;
  // double ckin4=3.0; //Definition moved to the function call
  ...
  // decayPy8->Set("321:mayDecay = 0"); //Let's see if this works.
  // decayPy8->Set("211:mayDecay = 0"); //Let's see if this works.
  ...
  filter = new D0JetFilter("partfilt");
  ...
  while (true) {
    ...
    for ( int _s=0;_s<stride;_s++) {
      getNextRun( event, runnumber, x, y, z ); 
      if ( 0==runnumber ) {
    ...
    cout << "Run # " << runnumber << "\t" << "Event # " << event << endl;

    trig( event, runnumber, x, y, z );
